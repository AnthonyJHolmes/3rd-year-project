<%-- 
    Document   : addNotif
    Created on : 03-Mar-2015, 11:51:51
    Author     : ajh3
--%>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            //setting up the database
                                ...
                                //if they url attacked they will be redirected
                                if(accept ==null && deny ==null){
                                    response.sendRedirect("notifications.jsp");
                                }
                                else if(accept==null){//user clicked deny
                                    db.Delete("DELETE FROM notifications WHERE user_id = "+userID+" AND group_name = '"+group+"';");
                                    out.println("You have refused to join the group: " + group);
                                }else if(deny==null){//user clicked accept so the notification is deleted and the user is added to the group
                                    int year =0;
                                 String yearString[] = db.SelectRow("SELECT year FROM notifications WHERE user_id = "+userID+" AND group_name = '"+group+"';");
                                 for(String s : yearString){
                                     year = Integer.parseInt(s);
                                 }
                                 db.Delete("DELETE FROM notifications WHERE user_id = "+userID+" AND group_name = '"+group+"';");
                                 db.Insert("INSERT INTO Groups (Group_name, ID_number, Year)"
                                         + "VALUES('"+group+"',"+userID+","+year+" );");
                                 out.println("Added to group: "+group+ ". You can now make group events with the people in the group ");
                                }
                             %>
